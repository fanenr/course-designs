...
#  define setsockopt(a, b, c, d, e)                                           \
    FreeRTOS_setsockopt ((a), (b), (c), (d), (e))
#  define sendto(a, b, c, d, e, f)                                            \
    FreeRTOS_sendto ((a), (b), (c), (d), (e), (f))
...
#  define MG_EPOLL_ADD(c)                                                     \
	...
	epoll_ctl (c->mgr->epoll_fd, EPOLL_CTL_ADD, (int) (size_t) c->fd,     \
		   &ev);                                                      \
      }                                                                       \
    while (0)
#  define MG_EPOLL_MOD(c, wr)                                                 \
	...
	if (wr)                                                               \
	  ev.events |= EPOLLOUT;                                              \
	epoll_ctl (c->mgr->epoll_fd, EPOLL_CTL_MOD, (int) (size_t) c->fd,     \
		   &ev);                                                      \
      }                                                                       \
    while (0)
#else
...
#    elif (MG_UECC_PLATFORM == mg_uecc_x86_64                                 \
	   || MG_UECC_PLATFORM == mg_uecc_arm64)
...
#  define MG_IRAM
  ...
  uint32_t
  mg_ota_timestamp (int firmware);   // Firmware timestamp, UNIX UTC epoch
...
#  define MG_PROF_ADD(c, name_)                                               \
	...
	struct mg_profitem item                                               \
	    = { name_, (uint32_t) mg_millis () - inittime };                  \
